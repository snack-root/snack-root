<!-- 🌃 CYBERPUNK RED TEAM README :: OPERATOR PROFILE -->

<h1 align="center">
  <span style="color:#00ffe0">```txt
 ███████╗███╗   ██╗ █████╗  ██████╗██╗  ██╗
 ██╔════╝████╗  ██║██╔══██╗██╔════╝██║ ██╔╝
 ███████╗██╔██╗ ██║███████║██║     █████╔╝
 ╚════██║██║╚██╗██║██╔══██║██║     ██╔═██╗
 ███████║██║ ╚████║██║  ██║╚██████╗██║  ██╗
 ╚══════╝╚═╝  ╚═══╝╚═╝  ╚═╝ ╚═════╝╚═╝  ╚═╝
 CALLSIGN: SNACK
```
</span>
</h1>

<p align="center">
  <em style="color:#7df9ff">
    Red Team Operator • Offensive Security • CTF Player
  </em>
</p>

<!-- STATUS / HUD -->
<p align="center">
  <img src="https://img.shields.io/badge/STATUS-ACTIVE-ff2bdc?style=for-the-badge&logo=ghost&logoColor=black"/>
  <img src="https://img.shields.io/badge/FOCUS-RED%20TEAM-00ffe0?style=for-the-badge&logo=kalilinux&logoColor=black"/>
  <img src="https://img.shields.io/badge/MODE-STEALTH--FIRST-39ff14?style=for-the-badge&logo=protonvpn&logoColor=black"/>
</p>

---

## 🧠 PROFILE

Cybersecurity practitioner operating inside **enterprise and lab environments**,  
with a strong focus on **offensive security**, **Red Team tradecraft**, and **CTF problem-solving**.

I spend most of my time breaking and understanding:
- Windows domains and **Active Directory trust relationships**
- Linux systems used for **offensive operations**
- Real-world attack chains from **initial access → post-exploitation**

My learning style is **hands-on**:
I build labs, break them, automate attacks, and document failures.

I don’t chase tools.  
I chase **understanding**.

---

## ⚔️ OPERATIONAL FOCUS

<p align="center">
  <img src="https://img.shields.io/badge/INITIAL%20ACCESS-ff2bdc?style=for-the-badge&logo=mitre&logoColor=black"/>
  <img src="https://img.shields.io/badge/CREDENTIAL%20ACCESS-00ffe0?style=for-the-badge&logo=mitre&logoColor=black"/>
  <img src="https://img.shields.io/badge/LATERAL%20MOVEMENT-7df9ff?style=for-the-badge&logo=mitre&logoColor=black"/>
  <img src="https://img.shields.io/badge/POST--EXPLOITATION-ff00ff?style=for-the-badge&logo=mitre&logoColor=black"/>
</p>

- Adversary emulation & Red Team simulations  
- Active Directory attacks (Kerberos, NTLM, trust abuse)  
- Credential harvesting & privilege escalation  
- Post-exploitation & lateral movement  
- OPSEC-aware attack workflows  

---

## 🖥 SYSTEMS I USE

<p align="center">
  <img src="https://img.shields.io/badge/WINDOWS%20AD-00ffe0?style=for-the-badge&logo=windows&logoColor=black"/>
  <img src="https://img.shields.io/badge/KALI%20LINUX-ff2bdc?style=for-the-badge&logo=kalilinux&logoColor=black"/>
  <img src="https://img.shields.io/badge/ARCH%20LINUX-00eaff?style=for-the-badge&logo=archlinux&logoColor=black"/>
  <img src="https://img.shields.io/badge/POP!__OS-7df9ff?style=for-the-badge&logo=popos&logoColor=black"/>
</p>

- Arch Linux & Pop!_OS for daily driver / labs  
- Kali Linux for offensive tooling  
- Windows environments for AD testing and exploitation  

---

## 🧰 TOOLCHAIN

<p align="center">
  <img src="https://img.shields.io/badge/METASPLOIT-ff2bdc?style=for-the-badge&logo=metasploit&logoColor=black"/>
  <img src="https://img.shields.io/badge/IMPACKET-00ffe0?style=for-the-badge&logo=python&logoColor=black"/>
  <img src="https://img.shields.io/badge/BLOODHOUND-ff00ff?style=for-the-badge&logo=neo4j&logoColor=black"/>
  <img src="https://img.shields.io/badge/NMAP-7df9ff?style=for-the-badge&logo=nmap&logoColor=black"/>
  <img src="https://img.shields.io/badge/BURP%20SUITE-ff8c00?style=for-the-badge&logo=burpsuite&logoColor=black"/>
  <img src="https://img.shields.io/badge/DOCKER-00eaff?style=for-the-badge&logo=docker&logoColor=black"/>
</p>

Tools change.  
Fundamentals don’t.

---

## 🧬 SKILLS & PRACTICE

<p align="center">
  <img src="https://img.shields.io/badge/PYTHON-00ffe0?style=for-the-badge&logo=python&logoColor=black"/>
  <img src="https://img.shields.io/badge/BASH-ff2bdc?style=for-the-badge&logo=gnu-bash&logoColor=black"/>
  <img src="https://img.shields.io/badge/CTF-7df9ff?style=for-the-badge&logo=hackthebox&logoColor=black"/>
  <img src="https://img.shields.io/badge/LABS-ff00ff?style=for-the-badge"/>
</p>

- Python & Bash scripting for automation  
- CTFs (crypto, web, pwn, misc) for skill sharpening  
- Custom labs for AD and network attacks  
- Linux & Windows internals exploration  

---

## 🧿 MINDSET

<p align="center">
  <img src="https://img.shields.io/badge/NOISE-MINIMIZED-39ff14?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/LEARN%20BY-BREAKING-ff2bdc?style=for-the-badge"/>
  <img src="https://img.shields.io/badge/DEFENDER-AWARE-00ffe0?style=for-the-badge"/>
</p>

I believe real security skills come from:
- Building systems yourself  
- Breaking them methodically  
- Understanding **why** they failed  

---

## 🌐 CONTACT

<p align="center">
  <a href="https://github.com/DangVinh2005">
    <img src="https://img.shields.io/badge/GITHUB-00ffe0?style=for-the-badge&logo=github&logoColor=black"/>
  </a>
  <a href="https://www.linkedin.com/in/dangvinh05/">
    <img src="https://img.shields.io/badge/LINKEDIN-ff2bdc?style=for-the-badge&logo=linkedin&logoColor=black"/>
  </a>
  <a href="mailto:dqv1507@gmail.com">
    <img src="https://img.shields.io/badge/EMAIL-7df9ff?style=for-the-badge&logo=gmail&logoColor=black"/>
  </a>
</p>

---

<sub style="color:#7df9ff">© Đặng Vinh · Red Team · Cyberpunk Operator</sub>
